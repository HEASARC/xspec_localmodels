...
#define JMAXP JMAX+1
#define K 5
#define FUNC_si(x) ((*func)(x))
#define SWAP_si(a,b) {double temp=(a);(a)=(b);(b)=temp;}
...
double (*func)();
{
   ...
   h[1]=1.0;
   for (j=1;j<=JMAX;j++) {
      s[j]=trapzd_si(func,a,b,j);
      if (j >= K) {
         polint_si(&h[j-K],&s[j-K],K,0.0,&ss,&dss);
...
int n;
{
   ...
   if (n == 1) {
      it=1;
      return (s=0.5*(b-a)*(FUNC_si(a)+FUNC_si(b)));
   } else {
      tnm=it;
      del=(b-a)/tnm;
      x=a+0.5*del;
      for (sum=0.0,j=1;j<=it;j++,x+=del) sum += FUNC_si(x);
      it *= 2;
      s=0.5*(s+(b-a)*sum/tnm);
      return s;
   }
...
int n,m;
{
   ...
   for (i=1;i<=n;i++) {
      ...
      ++(ipiv[icol]);
      if (irow != icol) {
         for (l=1;l<=n;l++) SWAP_si(a[irow][l],a[icol][l])
         for (l=1;l<=m;l++) SWAP_si(b[irow][l],b[icol][l])
      }
      indxr[i]=irow;
      indxc[i]=icol;
...
(float *ear,int ne,float *param,int ifl,float *photar,float *photer)
{
  ...
  if (lines) {
	...
	if (Nion[element][electron] && fMtemp && LINE <= 4) {
	  ...
	  line_opacity_si(Nion[element][electron],E0,OSCILLATOR,ALPHA,DELTANUD,tau_exc);
	}
      } else if (electron == 2) {
  ...
  for (element=10;element<=28;++element) {
	...
	while (fscanf(input,"%d%lf%d%lf%lf%lf",&i,&g_i,&j,&g_j,&THRESHOLD,&ANGULAR) != EOF) {
	  ...
	  spline_si(LOWE_EGRID,LOWE_PIGRID,LOWE_GRIDNUM,1.e40,1.e40,LOWE_PIGRID_2);	  
	  if (Nion[element][electron]*pisigma_si(g_i,p0,p1,p2,p3,THRESHOLD) >= tau_lim) {
	    ...
	    spline_si(EGRID,PIGRID,GRIDNUM,1.e40,1.e40,PIGRID_2);
...
double pispline_si(double E)
{
  ...
  E=log10(E);
  splint_si(EGRID,PIGRID,PIGRID_2,GRIDNUM,E,&answer);
...
double lowEpispline_si(double E)
{
  ...
  E=log10(E);
  splint_si(LOWE_EGRID,LOWE_PIGRID,LOWE_PIGRID_2,LOWE_GRIDNUM,E,&answer);
  return pow(10.,answer);
}
...
void line_opacity_si(double Nion_column_density,double E0,double OSCILLATOR,double ALPHA,double DELTANUD,double tau_exc_p[])
{
  ...
  line_limits_si(Nion_column_density,E0,OSCILLATOR,ALPHA,DELTANUD,&Elo,&Ehi);
