...
typedef int (*evalError_func)(
    struct rule_s* r, unsigned fdim, integrand_v f, void* fdata, unsigned nR, region* R);
...
static void evalR0_0fs4d(
    double* pts, unsigned dim, double* p, const double* c, const double* r1, const double* r2)
{
    unsigned i;
...
#define VALS(i) v[fdim * (i)]
        for (iR = 0; iR < nR; ++iR) {
            ...
            result = R[iR].h.vol * (r->weight1 * val0 + weight2 * sum2 + r->weight3 * sum3 +
...
static int rule15gauss_evalError(
    rule* r, unsigned fdim, integrand_v f, void* fdata, unsigned nR, region* R)
{
...
#include "converged.h"
    ...
    static int rulecubature(rule* r,
                            unsigned fdim,
                            integrand_v f,
                            void* fdata,
                            const hypercube* h,
...
static int cubature(unsigned fdim,
                    integrand_v f,
                    void* fdata,
                    unsigned dim,
                    const double* xmin,
                    ...
                    int parallel)
{
    ...
    status = !h.data ? FAILURE : rulecubature(r, fdim, f, fdata, &h, maxEval, reqAbsError,
